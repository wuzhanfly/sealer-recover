...
func GetSectorCommitInfoOnChain(ctx context.Context, fullNodeApi v0api.FullNode, maddr address.Address, sid abi.SectorNumber) (*types.TipSet, *miner.SectorPreCommitOnChainInfo, error) {
	...
	fmt.Printf(si.SectorNumber.String(),si.SealedCID.String())
	if err != nil {
		return nil, nil, err
	}
	...
	if si == nil {
		...
		fmt.Println(si.SectorNumber.String(),si.Activation.String(),si.SealedCID.String())
	}
	...
	fmt.Println("ts.key():",ts.Key().String())
